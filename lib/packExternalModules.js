          ...
          .then(dependencyGraph => {
            ...
            return BbPromise.fromCallback(cb => fse.pathExists(packageLockPath, cb))
              ...
              .mapSeries(compileStats => {
                ...
                // Skip this package if there are no external dependencies
                ...
                return BbPromise.try(() => {
                  ...
                  .then(() =>
                    // Remove lockfile after install
                    hasPackageLock ?
                      BbPromise.fromCallback(callback => fse.remove(path.join(modulePath, packager.lockfileName), callback)) :
                      BbPromise.resolve()
                  );
              })
              .return();
